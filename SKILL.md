...
name: jupyterlab-cli
description: >-
  jupyterlab-cli is a stateless HTTP client to the Jupyter server extension: configure
  ...
  from the terminal or agents, coordinating locks via JUPYTERLAB_CLI_SESSION, or
  avoiding local notebook state (all state on the server).
---

# jupyterlab-cli (agent skill)
...
Run **JupyterLab** (or Jupyter Server) in an environment where `jupyterlab-cli` is installed so the **`jupyterlab_cli_extension`** is available. Then use the `jupyterlab-cli` binary to talk to that server over HTTP.

## CLI shape

```text
jupyterlab-cli [OPTIONS] COMMAND [ARGS]...
```
...
| Option | Role |
|--------|------|
| `--json` | Machine-readable JSON (place **before** the subcommand, e.g. `jupyterlab-cli --json status`) |
| `--server-url TEXT` | Jupyter server base URL (overrides config / env) |
| `--token TEXT` | Auth token |
| `--session TEXT` | Stable session id for multi-client locking (or `JUPYTERLAB_CLI_SESSION`) |
...
Subcommand-specific flags: **`jupyterlab-cli <command> --help`**. Deep reference: **`README.md`**.

## Typical agent flow

1. **`jupyterlab-cli configure`** (or env: server URL + token).
2. Optionally **`export JUPYTERLAB_CLI_SESSION=...`** for a stable lock identity.
3. **`jupyterlab-cli use-notebook <path/to/notebook.ipynb>`** to attach and lock.
4. Mutate or run cells with the cell / execute commands above.
5. **`jupyterlab-cli unuse-notebook`** when finished.
...
- Repo: [github.com/wenmin-wu/jupyterlab-cli](https://github.com/wenmin-wu/jupyterlab-cli)
